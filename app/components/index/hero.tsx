...
const blogResources: BlogPost[] = [
  {
    ...
    subtitle: "Reduce stress, boost focus, and find balance through mindfulness.",
...
export default function IndexHero() {
  ...
  const getNextIndex = () => 
    (currentPostIndex + 1) % blogResources.length;

  return (
    <div className="relative h-screen mx-auto overflow-hidden">
      <div className="absolute inset-0">
        {/* Image Slides */}
        {[getPreviousIndex(), currentPostIndex, getNextIndex()].map((index, i) => (
          <img
            key={`slide-${index}`}
        ...
        {/* Content */}
        <div className={`absolute inset-0 flex h-full items-end justify-start transition-opacity duration-500 ${
        ...
        {/* Author Info */}
        <div className={`absolute inset-0 flex h-full items-end justify-end transition-opacity duration-500 ${
          isAnimating ? "opacity-0" : "opacity-100"
        }`}>
